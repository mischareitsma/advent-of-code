import os
import sys
import math

TEST: bool = "-t" in sys.argv
DAY: str = os.path.basename(__file__)[3:5]

def info_from_args() -> tuple[str, int]:
    n: str = ""
    p: int = 0
    for arg in sys.argv:
        if arg.startswith("-n"):
            n = arg[2:]
        if arg.startswith("-p"):
            p=int(arg[2])
    if p not in (0, 1, 2):
        p = 0
    return n, p

N, P = info_from_args()

FILE_NAME = f"day{DAY}{"_test" if TEST else ""}_input{N}.dat"
FILE_PATH = f'{os.path.dirname(os.path.realpath(__file__))}/{FILE_NAME}'

with open(FILE_PATH, "r") as f:
    LINES: tuple[str, ...] = tuple(_.strip('\n') for _ in f.readlines())

TLINES = []
for i in range(len(LINES[0])):
    l = ''
    for j in range(len(LINES)):
        l += LINES[j][i]
    TLINES.append(l)
# Why this no work?
# TLINES=tuple(zip(*LINES))
# print('\n'.join(TLINES))

def print_msg():
    msg: str = f"Running day {int(DAY)}"
    if P > 0:
        msg += f" part {P}"
    if TEST:
        msg += " in test mode"

    print(msg)
    print(f"Input file used for solution: {FILE_NAME}")

def common():
    pass

def part1():
    operations = LINES[-1].split()
    numbers = tuple(tuple(int(i) for i in r.split()) for r in LINES[:-1])
    numbers = tuple(zip(*numbers))
    n = 0
    for i, v in enumerate(operations):
        oper = sum if v == "+" else math.prod
        n += oper(numbers[i])
    return n

def part2():
    oper = None
    digits = []
    n = 0

    for l in TLINES:
        if l.strip() == "":
            n += oper(digits)
            oper = None
            digits = []
            continue
        if not oper:
            oper = sum if l[-1] == "+" else math.prod
        digits.append(int(l[:-1]))
    n += oper(digits)
    return n
if __name__ == "__main__":
    print_msg()
    common()
    if P in (0, 1):
        print(f"part 1: {part1()}")
    if P in (0, 2):
        print(f"part 2: {part2()}")
